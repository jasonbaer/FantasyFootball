...
#  workingDirectory <- getwd()
  ...
  api_call <- paste('python \"', workingDirectory, "/", sourceFolder, '/fp_api.py\" -j \"', json_dest,'\" -c \"',csv_dest,'\" -y ',year,' -p ',position,' -w ',thisweek,' -s ',scoring, sep="")
...
download.data <- function(pos.list=c("rb","wr","te","flx"), scoring="STD") {
  ...
  if (download == TRUE) {
    for (mp in pos.list) {
      ...
      head.dir <- paste(datdir, "week-", sep="")
...
download.data.ecr = function(week, pos.list) {
  
  # week=1
  # pos.list=c('flx','rb','wr','te')

  for (mp in pos.list) {
    url <- paste("http://www.fantasypros.com/nfl/rankings/", mp, ".php?week=", thisweek, "&export.xls", sep="")
    dest <- paste(datdir, "week-", week, "-", mp, "-raw.xls", sep="")
    
    download.file(url, dest)  
    
    sedstr <- paste("sed '1,4d' ", datdir, "week-", week, "-", mp, "-raw.xls", 
                   " > ", datdir, "week-", week, "-", mp, ".tsv", sep="")
    system(sedstr)
  }	  
}


download.predraft.data <- function(dir) {
  datdir <- dir
  # overall rankings download:

  base_dest <- paste(datdir, 'week-0-ALL-STD-raw', sep="")
  json_dest <- paste(base_dest, '.txt', sep='')
  csv_dest <- paste(base_dest, '.csv', sep='')
  download.py.call(json_dest, csv_dest, position='ALL', scoring='STD')
  
  base_dest <- paste(datdir, 'week-0-ALL-PPR-raw', sep="")
  json_dest <- paste(base_dest, '.txt', sep='')
  csv_dest <- paste(base_dest, '.csv', sep='')
  download.py.call(json_dest, csv_dest, position='ALL', scoring='PPR')
  
  base_dest <- paste(datdir, 'week-0-ALL-HALF-PPR-raw', sep="")
  json_dest <- paste(base_dest, '.txt', sep='')
  csv_dest <- paste(base_dest, '.csv', sep='')
  download.py.call(json_dest, csv_dest, position='ALL', scoring='HALF')
}  
...
read_ecr_data <- function(path) {
  ...
  head.dir <- paste(datdir, "week-", sep="")
  csv_dest <- paste(head.dir, thisweek, "-", mp, "-", scoring, "-raw.csv", sep="")

  ecr_df <- read_csv(path)
...
draw.tiers <- function(pos='all', low=1, high=100, k=3, adjust=0, XLOW=0, highcolor=360, num.higher.tiers=0, scoring='STD', save=TRUE) {
  ...
  pos.scoring = paste(position, scoring, sep='-')
  tpos = pos.scoring
  head.dir = 'FantasyFootball/fftiers/dat/2018/week-'
  head.dir = paste(datdir, 'week-', sep="")
